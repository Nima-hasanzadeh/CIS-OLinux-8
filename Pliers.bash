...
function disable_fs {
 ...
 rmmod usb-storage
  ...
  install udf /bin/false blacklist freevxfs
  install udf /bin/false blacklist hfs
  install udf /bin/false blacklist hfsplus
  install udf /bin/false blacklist jffs2
  install tipc /bin/false blacklist tipc
  install tipc /bin/false blacklist dccp
  install tipc /bin/false blacklist sctp
  install tipc /bin/false blacklist rds
...
function set_aslr {
 grep -qi '^\s*#*kernel.randomize_va_space\s=\s2\b' ${SYSCTL_60} || echo -e "kernel.randomize_va_space = 2 " >> ${SYSCTL_60} || return
...
function cockpit {
 ...
 # checking Initial Setup
  ...
  echo_bold "##### 1.4.1 Ensure address space layout randomization (ASLR) is enabled #####"
   ...
   echo_bold "#####  1.4.2 Ensure ptrace_scope is restricted  #####"
	f_return apply_sysctl kernel.yama.ptrace_scope 1
	replace_parm_nospace "kernel.yama.ptrace_scope =" 1 ${SYSCTL_60}

   echo_bold "##### 1.4.3 - 4 Ensure core dump restricted #####"
   f_return  core_dump_conf

  echo_bold "##### 1.5.1.1 - 3  Ensure SELinux is enabled and configured #####"
   f_return selinux

 echo_bold "##### 1.5.1.7 - 8 Remove SETroubleshoot and MCS Translation #####"
 ...
 echo_bold "##### 2.2.1 - 22 Removing lagacy services . . .  "
...
echo_bold "##### 4.2.1 Config rsyslog . . ."
 ...
 echo_bold "##### 5.2.5 - 20 Configure SSHD Config #####"
  ...
  f_return replace_parm disableforwarding yes ${SSHD_CFG}
  ...
  f_return replace_parm Ciphers aes128-ctr,aes192-ctr,aes256-ctr ${SSHD_CFG}
 ...
 echo_bold "##### 5.3.7 Ensure access to the su command is restricte #####"
  ...
  echo_bold "##### 5.4.1 - 2 Ensure authselect includes with-faillock , pwhistory ..  . #####"
 ...
 echo_bold "##### 5.6.1.2 Ensure minimum days between password changes is 7 or more #####"
  f_return replace_parm PASS_MIN_DAYS 7   ${LOGIN_DEFS} 
 ...
 echo_bold "##### 6.1 set history time format #####"
  ...
  f_return audit_sys_rpm
   ...
   echo_bold "##### 6.1.2 Ensure sticky bit is set on all world-writable directories #####"
  f_return sticky_bit

 echo_bold "##### 6.1.1 - 10 Ensure permissions on passwd(-), group(-) and shadow(-) files are configures #####"
 ...
 echo_bold "##### 6.1.11 Ensure no world writable files exist (Manual) #####"
  f_return world_writable_files

 echo_bold "##### 6.1.12 Ensure no unowned files or directories exist (Manual) #####"
  f_return unowned_files

 echo_bold "##### 6.1.13 Ensure no ungrouped files or directories exist (Manual) #####"
  f_return ungrouped_files
 
 echo_bold "##### 6.1.14 Audit SUID executables (Manual) #####"
  f_return SUID_executables
 
 echo_bold "##### 6.1.15 Audit SGID executables (Manual) #####"
  f_return SUID_executables
 ...
 echo_bold "##### 6.2.3 Ensure no duplicate UIDs exist (Manual) #####"
  f_return duplicate_UID
 
 echo_bold "##### 6.2.4 Ensure no duplicate GIDs exist (Manual) #####"
  f_return duplicate_GID

 echo_bold "##### 6.2.5 Ensure no duplicate user names exist (Manual) #####"
  f_return duplicate_username

 echo_bold "##### 6.2.6 Ensure no duplicate group names exist (Manual) #####"
  f_return duplicate_groupname

 echo_bold "##### 6.2.7 Ensure root PATH Integrity (Manual) #####"
  f_return root_path

 echo_bold "##### 6.2.8 Ensure root is the only UID 0 account #####"
  f_return root_uid

 echo_bold "##### 6.2.9 Ensure local interactive user home directories exist #####"
  f_return home_dirs_exist 

 echo_bold "##### 6.2.10 Ensure local interactive users own their home directories #####"
  f_return home_dirs_owner

 echo_bold "##### 6.2.11 Ensure local interactive user home directories are mode 750 or more restrictive #####"
  f_return home_dirs_perm

  echo_bold "##### 6.2.12 Ensure local interactive user dot files are not group or world writable #####"
  f_return dot_files
