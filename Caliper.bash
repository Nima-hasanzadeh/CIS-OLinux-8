...
#Author: Nima.H 
#OS Support : Oracle Linux 8 (It may be compatible with most Red Hat distributions)
...
read -p "Press Enter to continue . . ."
 ...
 if echo ${PRETTY_NAME} | cut -f1 -d'.' | grep -q "Oracle Linux Server 8"; then
  echo
    else
...
clear
 ...
 echo_bold "##### 1.3.2 Ensure permissions on bootloader config are configured"
   f_return check_grub_owns
   f_return check_file_perms ${GRUB_CFG} 700
   f_return check_file_perms ${GRUB_ENV} 600
  
   echo_bold "##### 1.4.1 Ensure address space layout randomization (ASLR) is enabled #####"
    f_return chk_sysctl kernel.randomize_va_space 2
    f_return chk_aslr
	
   echo_bold "#####  1.4.2 Ensure ptrace_scope is restricted  #####"
	f_return chk_sysctl kernel.yama.ptrace_scope 1
	f_return chk_param "${SYSCTL_CONF}" "kernel.yama.ptrace_scope =" 1

   echo_bold "##### 1.4.3 Ensure core dump backtraces are disabled  #####"
   f_return restrict_bcktrc_dumps 
   
   echo_bold "##### 1.4.4 Ensure core dump restricted #####"
   f_return restrict_core_dumps 
  
  echo_bold "##### 1.5.1.1 Ensure SELinux is installed #####"
   f_return rpm_installed libselinux
   
  echo_bold "##### 1.5.1.2 Ensure SELinux is not disabled in bootloader configuration #####"
   f_return verify_selinux_grubcfg
   
  echo_bold "##### 1.5.1.4 Ensure the SELinux mode is not disabled #####"
   f_return verify_selinux_state

  echo_bold "##### 1.5.1.7 Ensure SETroubleshoot is not installed #####"
   f_return rpm_not_installed setroubleshoot 
  echo_bold "##### 1.5.1.8 Ensure the MCS Translation Service (mcstrans) is not installed #####"
   f_return rpm_not_installed mcstrans
   
   echo_bold "##### 1.6.1 Ensure system-wide crypto policy is not legacy #####"
   f_return chk_cryptopolicy_not_legacy
  
  echo_bold "##### 1.6.2 Ensure system wide crypto policy disables sha1 hash and signature support "
   f_return  disable_sha1
   
  echo_bold "##### 1.6.3 Ensure system wide crypto policy disables cbc for ssh  #####"
    f_return disable_cbc

  echo_bold "##### 1.6.4 Ensure system wide crypto policy disables macs less than 128 bits #####"
   f_return disable_oldmac
...
#checking Servicess Configuration
  ...
  echo_bold "##### 2.1.3 Ensure chrony is not run as the root user  #####"
   f_return  chrony_user

  echo_bold "##### 2.2.2 Ensure X Window System not installed #####"
   f_return rpm_not_installed xorg-x11-server-common

  echo_bold "##### 2.2.1-22 Ensure unused services not installed #####"
   f_return rpm_not_installed autofs
 ...
 # Checking Network Configuration
  ...
  echo_bold "##### 3.3.1 Verify if IPv6 is disabled on the sys_ctl #####" 
  ...
  echo_bold "##### 3.1.3 Ensure bluetooth services are not in use #####"
  f_return rpm_not_installed bluez
     
  echo_bold "##### 3.2.1 Ensure dccp kernel module is not available #####"
     f_return disable_fs dccp
  
  echo_bold "##### 3.2.2 Ensure tipc kernel module is not available #####"
       f_return disable_fs tipc
  
  echo_bold "##### 3.2.3 Ensure rds kernel module is not available  #####"
       f_return disable_fs rds

  echo_bold "##### 3.2.4 Ensure sctp kernel module is not available #####"
 ...
 echo_bold "##### 3.3.2 Ensure packet redirect sending disabled #####"
...
echo_bold "##### 5.1.1 Ensure cron daemon is enabled #####"
 ...
 echo_bold "##### 5.2.2 Ensure permissions on SSH private host key files"
 ...
 echo_bold "##### 5.2.3 Ensure permissions on SSH public host key files"
 ...
 echo_bold "##### 5.2.5-20 Ensure SSH options are set properly"
  ...
  f_return chk_param "${SSHD_CFG}" disableforwarding yes
  ...
  f_return chk_param "${SSHD_CFG}" Ciphers aes128-ctr,aes192-ctr,aes256-ctr


 echo_bold "##### 5.2.5-21 Ensure SSH options are set properly - Second Check"
  ...
  f_return chk_ssh_conf2  disableforwarding yes
  ...
  f_return chk_ssh_conf2  aes128-ctr,aes192-ctr,aes256-ctr
...
echo_bold "##### 5.5.1 Ensure password creation req. configured"
   ...
   f_return chk_parm_2 "${PWQUAL_CNF}" maxsequence 3

  echo_bold  "##### 5.5.2 Configure pam_faillock module"
  ...
  echo_bold "##### 5.5.3 Ensure password reuse is limited"
   ...
   f_return chk_parm_2 "${PWHISTORY_CNF}" remember 5

  echo_bold "##### 5.5.4 Ensure password hashing algo is SH512"
   f_return pass_hash

   echo_bold "##### 5.6.1.1 - 3 Ensure password config"
   f_return chk_param "${LOGIN_DEFS}" PASS_MAX_DAYS 365
   f_return chk_param "${LOGIN_DEFS}" PASS_MIN_DAYS 7
 ...
 echo_bold "##### 6.1.2 Ensure sticky bit set on all world-writeable dirs"
   f_return sticky_wrld_dirs 
            sticky_wrld_dirs

  echo_bold "##### 6.1.3 - 10 Ensure perms on passwd(-), group(-) and shadow(-) files"
  ...
  echo_bold "##### 6.1.11 Ensure no world writable files exist (777)"
    f_return world_perm 
             world_perm     
  
  echo_bold "##### 6.1.12 Ensure no unowned files exist"
   f_return unowned_files	
             unowned_files

  echo_bold "##### 6.1.13 Ensure no ungrouped files exist"
   f_return ungrouped_files
	        ungrouped_files

   echo_bold "##### 6.1.14 Audit SUID executables"
   f_return suid_exes
  
  echo_bold "##### 6.1.15 Audit SGID executables"
   f_return sgid_exes
  
  echo_bold "##### 6.2.0 Ensure accounts in /etc/passwd use shadowed passwords"
   f_return passwd_shadow
  	   
  echo_bold "##### 6.2.1 Ensure password  fields are not empty"
   f_return passwd_field

  echo_bold "##### 6.2.2 Ensure all groups in /etc/passwd exist in /etc/group "
   f_return groups_passwd

  echo_bold "##### 6.2.3 Ensure no duplicate UIDs exist"
   f_return duplicate_uids

  echo_bold "##### 6.2.4 Ensure no duplicate GIDs"
   f_return duplicate_gids

  echo_bold "##### 6.2.5 Ensure no duplicate user names"
   f_return duplicate_usernames

  echo_bold "##### 6.2.6 Ensure no duplicate group names"
   f_return duplicate_groupnames

  echo_bold "##### 6.2.7 Ensure root PATH integrity"
   f_return root_path

  echo_bold "##### 6.2.8 Ensure root is the only UID 0 account"
   f_return no_uid0_other_root

  echo_bold "##### 6.2.9 Ensure all users home dir exist"
   f_return chk_home_dirs_exist

  echo_bold "##### 6.2.10 Ensure users own their home directories"
   f_return chk_home_dirs_owns

  echo_bold "##### 6.2.11 Ensure users home directories permissions are 750 or more restrictive"
   f_return home_dir_perms
            home_dir_perms

echo_bold "##### 6.2.12 Ensure users dot files are not group or world writable"
